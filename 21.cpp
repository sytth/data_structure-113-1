...
public:
    ...
    bool isForest() {
        bool visited[count] = {false};	// 記錄節點是否訪問過
        bool hasCycle = false;			// 用來記錄是否有循環
        
        for(int i = 0; i < count; i++) {
            if (!visited[i]) {
                if (dfs((*vertex)[i].getData(), visited, NULL)) {
                    hasCycle = true;   // 發現循環，返回 false
                    break;
                }
            }
        }
        
        return !hasCycle;  // 如果有循環返回 false，否則返回 true
    }

    // DFS 函數，回傳是否有循環
    bool dfs(GraphNode<T> *node, bool visited[], GraphNode<T> *parent) {
        visited[node->getData() - 'A'] = true; // 標記訪問過
        
        ListNode<GraphNode<T> *> *adjNode = node->getLinks()->getHead(); // 取得相鄰節點
        while (adjNode != NULL) {
            if (!visited[adjNode->getData()->getData() - 'A']) {   // if 當前的相鄰節點未被訪問過，遞迴調用 dfs
                if (dfs(adjNode->getData(), visited, node)) {
                    return true;  // 發現循環，回傳 true
                }
            } else if (adjNode->getData() != parent) {
                return true;  	  // if 已訪問過但不是當前節點的父節(上一節點)，代表有循環
            }
            adjNode = adjNode->getNext(); // 繼續搜尋相鄰節點
        }
        return false;
    }
